...
`.github/workflows/k6.yml` runs **weekly** (Monday), **on demand**, and on pushes that touch the perf suite itself — deliberately *not* on every commit. It installs k6 via the official `grafana/setup-k6-action`, runs the smoke test
